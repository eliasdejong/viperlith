...
1. [Datastar](https://data-star.dev/) hypermedia framework.
...
9. [spsc-ring-threadsafe](https://pypi.org/project/spsc-ring-threadsafe): A ring buffer C extension for inter-process communication on shared memory, up to 100x faster than the standard library's `multiprocessing.Queue`. Used to queue commands to the single-writer process with exclusive write access to SQLite.
